...
using System.Linq;
using System.Text;
using Teal.WeChat.Helper;
using Codeplex.Data;
using System.Net.Http;

namespace Teal.WeChat.Card
{
    /// <summary>
    /// 创建卡券接口
    /// </summary>
    public class CreateCardAPI
    {
        ...
        public static string UploadLogo(string access_token, string file, string proxy = null)
        {
            ...
            var returnMessage = DynamicJson.Parse(Util.HttpRequestPost(url, fileDictionary, proxy:proxy));
            if (returnMessage.IsDefined("errcode")) return string.Empty;
        ...
        public static dynamic CreateCard(string access_token, dynamic card)
        {
            ...
            var client = new HttpClient();
            var req = DynamicJson.Serialize(card);
            var result = client.PostAsync(url, new StringContent(req)).Result;
            if (!result.IsSuccessStatusCode) return null;
            return DynamicJson.Parse(result.Content.ReadAsStringAsync().Result);
        }

        public static string CreateCard(string access_token, string req, string proxy =null) {
            var url = string.Format("https://api.weixin.qq.com/card/create?access_token={0}", access_token);
            return Teal.Utility.NetHelper.request(url, req, proxy: proxy);
        }
